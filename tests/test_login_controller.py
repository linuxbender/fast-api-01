...
class TestLoginControllerLogin:
    """Tests for POST /login endpoint."""

    def test_login_route_exists(self):
        """Test that login route is registered."""
        # Arrange
        controller = LoginController(APIRouter())

        # Act - Find the login route
        login_route = None
        for route in controller.router.routes:
    ...
    def test_login_route_sets_cookie(self):
    ...
    def test_login_status_code_is_200(self):
        """Test login returns 200 OK status code."""
        # Arrange
        controller = LoginController(APIRouter())
        ...
        # Assert
        assert login_route is not None
        assert hasattr(login_route, "status_code")
        assert login_route.status_code == status.HTTP_200_OK

    def test_login_route_has_correct_metadata(self):
        """Test login route has correct summary and description."""
        # Arrange
        controller = LoginController(APIRouter())
        ...
        # Assert
        assert login_route is not None
        assert hasattr(login_route, "summary")
        assert login_route.summary == "Login"
        assert hasattr(login_route, "description")
        assert "email" in login_route.description.lower()
        assert "password" in login_route.description.lower()
